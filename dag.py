...
class DAG(object):
    ...
    def get_augmented_samples(self, x):
        ...
        return x_arg
#lry change;add cc,nc
    def compute_discriminator_loss(self, x_real, x_fake,cc,nc, netD,grad_penalty_weight=10):
...
#                 d_loss_per_policy += self.D_loss_func(x_real_aug[j], x_fake_aug[j],cc,nc,netD,grad_penalty_weight,dag=True, dag_idx=policy_index+j)
                d_loss_per_policy += self.D_loss_func(x_real_aug, x_fake_aug,cc,nc,netD,grad_penalty_weight,dag=True, dag_idx=policy_index+j)#lry change
            d_loss = d_loss + weight * d_loss_per_policy / n_policy
            policy_index += n_policy
        return d_loss
#lry change
    def compute_generator_loss(self, x_real, x_fake,cc,nc, netD):
