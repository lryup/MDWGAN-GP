...
def pearson_correlation(x, y):
    ...
    #lry change,because std is 0,final values is nan
    def standardize(a):
        a_off = np.mean(a, axis=0)
        a_std = np.std(a, axis=0)
        adata=(a - a_off) / a_std
...
def plot_individual_distrs(x, y, symbols, nrows=4, xlabel='X', ylabel='Y'):
    ...
    for r in range(nrows):
        for c in range(ncols):
            ...
            if idx + 1 == nb_symbols:
                break
def lry_plot_individual_distrs(x, y, z,symbols, nrows=4, xlabel='X', ylabel='Y',zlabel='Z'):
    """
    Plots individual distributions for each gene
    """
    nb_symbols = len(symbols)
    ...
    for r in range(nrows):
        for c in range(ncols):
            ...
            plt.title(symbols[idx])
            plot_distribution(x[:, idx], xlabel='', ylabel='', label=xlabel, color='black')
            plot_distribution(y[:, idx], xlabel='', ylabel='', label=ylabel, color='red')
            plot_distribution(z[:, idx], xlabel='', ylabel='', label=zlabel, color='royalblue')
...
def plot_tf_activity_histogram(expr, gene_symbols, xlabel='Fraction of chips. TF activity', color='royalblue',
    ...
    ax.set_xscale('log', base=2)#lry change basex=2
    ax.set_xlim(2 ** -10, 1)
